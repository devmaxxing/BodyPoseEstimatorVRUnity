...
public class BodyRotationPoser : BodyPoser{
	...
	public override void AdjustPose() {
		...
		runner.AddInput (graph [inputLayer] [0], new float[,]{{ 	head.rotation.x, head.rotation.y, head.rotation.z, head.rotation.w, head.position.x * scalingFactor, head.position.y * scalingFactor, head.position.z *scalingFactor,
